...
export default function CatalogPage() {
 ...
 <span className="text-[11px] font-black text-danger uppercase tracking-wide">{lowStock.length} stock crítico</span>
 </div>
 )}

 {canManageProducts && subTab === "productos" && (
 <button onClick={openNewProduct} className="px-3 py-1.5 bg-brand-500 text-black rounded-lg text-[11px] font-black uppercase tracking-wide hover:bg-brand-400 transition-all shadow-lg shadow-brand-500/20 active:scale-95 shrink-0">
 + Nuevo Producto
 </button>
 )}
 ...
 placeholder="Nueva categoría..." className="h-7 px-2.5 bg-surface-2 dark:bg-white/5 border border-border/40 dark:border-white/5 rounded-lg text-[11px] font-bold focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-48" />
 <button onClick={createCategory} className="px-3 py-1.5 bg-brand-500 text-black rounded-lg text-[11px] font-black uppercase tracking-wide hover:bg-brand-400 transition-all active:scale-95 shrink-0">+ Crear</button>
 </div>
 )}
 </div>
 ...
 className={`px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wide transition-all ${subTab === key ? "bg-brand-500 text-black shadow-[0_0_12px_rgba(20,184,166,0.3)]" : "text-content-subtle dark:text-white/30 hover:text-content dark:hover:text-white hover:bg-surface-2 dark:hover:bg-white/5"}`}>
 {label}
 </button>
 ))}
 </div>
 ...
 placeholder="Buscar producto..." className="w-full h-7 pl-7 pr-3 bg-surface-2 dark:bg-white/5 border border-border/40 dark:border-white/5 rounded-lg text-[11px] font-bold focus:ring-2 focus:ring-brand-500/20 outline-none transition-all" />
 </div>
 <select value={selectedWarehouseId} onChange={e => setSelectedWarehouseId(e.target.value)}
 className="h-7 px-2.5 bg-surface-2 dark:bg-white/5 border border-border/40 dark:border-white/5 rounded-lg text-[11px] font-black focus:ring-2 focus:ring-brand-500/20 outline-none transition-all max-w-[180px]">
 ...
 <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30">Producto</th>
 <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30">Stock</th>
 <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30">Precio</th>
 {canManageProducts && <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30 text-right">Acciones</th>}
 </tr>
 </thead>
 <tbody className="divide-y divide-border/20 dark:divide-white/5">
 {products.length === 0 ? (
 <tr><td colSpan={5} className="p-10 text-center text-[11px] font-black uppercase tracking-wide opacity-20">Sin productos registrados</td></tr>
 ) : products.map(p => {
 ...
 : <div className="w-9 h-9 rounded-lg bg-surface-3 dark:bg-white/5 border border-border/20 dark:border-white/5 flex items-center justify-center text-[11px] font-black text-content-subtle dark:text-white/30">{p.name.charAt(0)}</div>
 }
 </td>
 ...
 <div className="text-[11px] font-bold text-content-subtle dark:text-white/30 mt-0.5 uppercase tracking-wide">{p.category_name || "Sin categoría"}</div>
 </td>
 <td className="px-4 py-2">
 <div className={`text-[11px] font-black tabular-nums ${stockColor(localStock)}`}>
 {localStock}
 <span className="text-[11px] ml-1 opacity-50 uppercase">{p.unit || "uds"}</span>
 </div>
 {hasLocal && globalStock !== localStock && (
 <div className="text-[11px] font-bold opacity-30 uppercase">Global: {globalStock}</div>
 )}
 ...
 <div className="text-[11px] font-bold opacity-30 uppercase">precio final</div>
 </td>
 {canManageProducts && (
 ...
 <span className="text-[11px] font-black uppercase tracking-wide opacity-30">{totalProducts} productos</span>
 <div className="flex items-center gap-1">
 ...
 <span className="text-[11px] font-black uppercase tracking-wide opacity-40 px-2">{page} / {totalPages}</span>
 ...
 <thead className="sticky top-0 z-10 bg-surface-2 dark:bg-surface-dark-2 backdrop-blur-md">
 <tr className="border-b border-border/30 dark:border-white/5">
 <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30">Nombre</th>
 <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30">Productos</th>
 {canManageCategories && <th className="px-4 py-2.5 text-[11px] font-black uppercase tracking-wide text-content-subtle dark:text-white/30 text-right">Acciones</th>}
 </tr>
 </thead>
 <tbody className="divide-y divide-border/20 dark:divide-white/5">
 {categories.length === 0 ? (
 <tr><td colSpan={3} className="p-10 text-center text-[11px] font-black uppercase tracking-wide opacity-20">Sin categorías</td></tr>
 ) : categories.map(cat => (
 ...
 className="h-7 px-2.5 bg-surface-2 dark:bg-white/10 border border-brand-500/30 rounded-lg text-[11px] font-black outline-none flex-1 max-w-xs" />
 ...
 <span className="text-[11px] font-black uppercase text-content dark:text-white tracking-tight group-hover:text-brand-500 transition-colors">{cat.name}</span>
 )}
 </td>
 <td className="px-4 py-2.5">
 <span className="px-2 py-0.5 rounded-full bg-brand-500/10 text-brand-500 text-[11px] font-black uppercase tracking-wide border border-brand-500/10">
