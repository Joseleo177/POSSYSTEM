...
function KpiCard({ label, value, sub, color = "text-warning" }) {
  return (
    <div className="bg-surface-2 dark:bg-white/[0.04] border border-border/30 dark:border-white/5 rounded-lg p-3">
      <div className="text-[7.5px] font-black uppercase tracking-[2px] text-content-subtle dark:text-white/30 mb-2">{label}</div>
      <div className={`text-base font-black tabular-nums leading-none ${color} mb-1`}>{value}</div>
      {sub && <div className="text-[8px] text-content-subtle dark:text-white/20 font-medium">{sub}</div>}
    </div>
  );
}
...
function SalesChart({ data }) {
  ...
  return (
    <div className="bg-surface-2 dark:bg-white/[0.04] border border-border/30 dark:border-white/5 rounded-lg p-3 flex flex-col">
      <div className="text-[7.5px] font-black uppercase tracking-[2px] text-content-subtle dark:text-white/30 mb-2">Ventas últimos 30 días</div>
...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div className="h-full flex items-center justify-center">
      <div className="text-[9px] font-black uppercase tracking-widest text-content-subtle dark:text-white/20 animate-pulse">Cargando dashboard…</div>
    </div>
  );
  if (error) return (
    <div className="h-full flex items-center justify-center text-danger text-[9px] font-black uppercase">{error}</div>
  );
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="shrink-0 px-4 pt-3 pb-2 flex items-center justify-between border-b border-border/30 dark:border-white/5">
        <div>
          <div className="text-[7px] font-black text-brand-500 uppercase tracking-[3px] leading-none mb-0.5">PANEL DE CONTROL</div>
          <h1 className="text-sm font-black text-content dark:text-white uppercase tracking-tight leading-none">Dashboard</h1>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[8px] font-bold text-content-subtle dark:text-white/30">
            {new Date().toLocaleDateString("es-VE", { weekday: "short", day: "numeric", month: "short" })}
          </span>
          <button onClick={load} className="px-2.5 py-1 bg-surface-2 dark:bg-white/5 border border-border/30 dark:border-white/5 rounded-lg text-[8px] font-black uppercase tracking-widest hover:text-brand-500 transition-all active:scale-95">
            ↻ Actualizar
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-auto p-3 space-y-3">
        ...
        {/* KPI — Hoy */}
        <div>
          <div className="text-[7px] font-black text-content-subtle dark:text-white/20 uppercase tracking-[3px] mb-1.5">HOY</div>
        ...
        {/* KPI — 30 días */}
        <div>
          <div className="text-[7px] font-black text-content-subtle dark:text-white/20 uppercase tracking-[3px] mb-1.5">ÚLTIMOS 30 DÍAS</div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-2">
          ...
          <div className="bg-surface-2 dark:bg-white/[0.04] border border-border/30 dark:border-white/5 rounded-lg p-3">
            <div className="text-[7.5px] font-black uppercase tracking-[2px] text-content-subtle dark:text-white/30 mb-2.5">TOP PRODUCTOS (30 DÍAS)</div>
            {top_products.length === 0
              ? <div className="text-[8px] text-content-subtle dark:text-white/20 text-center py-4">Sin datos</div>
        ...
        {low_stock.length > 0 && (
          <div className="bg-surface-2 dark:bg-white/[0.04] border border-danger/20 rounded-lg p-3">
            <div className="text-[7.5px] font-black text-danger uppercase tracking-[2px] mb-2">STOCK BAJO — REQUIERE ATENCIÓN</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {low_stock.map(p => (
                <div key={p.id} className="flex items-center justify-between bg-surface-1 dark:bg-white/5 border border-danger/10 rounded-lg px-3 py-2">
                  <div>
                    <div className="text-[9px] font-bold text-content dark:text-content-dark">{p.name}</div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[7px] text-content-subtle dark:text-white/30">{p.unit}</div>
                  </div>
                </div>
              ))}
            </div>
