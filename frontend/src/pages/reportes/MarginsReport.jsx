        ...
        <div className="flex-1 min-h-0 space-y-3 overflow-auto">
          ...
          <Card className="!p-0 overflow-hidden min-h-0 flex flex-col">
            ...
            <div className="overflow-x-auto">
                  ...
                  {view === "top" && (
                    <tr className="border-b border-border/40 dark:border-white/5">
                      {["Ranking", "Producto", "Ingresos", "Inversión", "Utilidad", "ROI %"].map(h => (
                        <th key={h} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest text-content-muted dark:text-content-dark-muted ${["Ingresos","Inversión","Utilidad"].includes(h) ? "text-right" : h === "ROI %" ? "text-center" : ""}`}>{h}</th>
                      ))}
                    </tr>
                  )}
                  {view === "category" && (
                    <tr className="border-b border-border/40 dark:border-white/5">
                      {["Categoría", "Volumen", "Utilidad", "ROI"].map(h => (
                        <th key={h} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest text-content-muted dark:text-content-dark-muted ${["Volumen","Utilidad"].includes(h) ? "text-right" : h === "ROI" ? "text-center" : ""}`}>{h}</th>
                      ))}
                    </tr>
                  )}
                </thead>
                <tbody className="divide-y divide-border/20 dark:divide-white/5">
                  {view === "top" && data.by_product.slice(0, 20).map((p, i) => (
                    <tr key={i} className="hover:bg-surface-2 dark:hover:bg-white/[0.04] transition-colors">
                      <td className="px-4 py-2"><div className="w-6 h-6 rounded-lg bg-surface-3 dark:bg-white/5 flex items-center justify-center text-[8px] font-black text-brand-500">{i + 1}</div></td>
                      <td className="px-4 py-2">
                        <div className="font-black text-[9px] uppercase tracking-wider text-content dark:text-white">{p.product_name}</div>
                        <div className="text-[8px] font-bold text-content-subtle opacity-50 uppercase">{p.category_name}</div>
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums text-[9px] text-content dark:text-white font-black">{fmt$(p.revenue)}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-danger text-[9px]">{fmt$(p.total_cost)}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-green-500 font-black text-[9px]">{fmt$(p.gross_margin)}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full ${parseFloat(p.margin_pct) >= 30 ? "text-green-500 bg-green-500/10" : "text-brand-500 bg-brand-500/10"}`}>
                          {p.margin_pct}%
                        </span>
                      </td>
                    </tr>
                  ...
                  {view === "category" && data.by_category.map((c, i) => (
                    <tr key={i} className="hover:bg-surface-2 dark:hover:bg-white/[0.04] transition-colors">
                      <td className="px-4 py-2 font-black text-[9px] uppercase tracking-wider text-brand-500">{c.category_name}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-[9px] text-content dark:text-white font-black">{fmt$(c.revenue)}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-green-500 font-black text-[9px]">{fmt$(c.gross_margin)}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full ${parseFloat(c.margin_pct) >= 25 ? "text-green-500 bg-green-500/10" : "text-brand-500 bg-brand-500/10"}`}>
                          {c.margin_pct}%
                        </span>
                      </td>
                    </tr>
              ...
              {view === "bottom" && <div className="p-10 text-center text-[9px] font-black uppercase tracking-[3px] text-content-subtle opacity-20">Análisis Completo en el Excel Estratégico</div>}
            </div>
          </Card>
        </div>
      )}
