    ...
    <div className="flex items-center gap-3 shrink-0">
      <div className="flex gap-1">
        {presets.map(p => (
          <button key={p.label} onClick={() => applyPreset(p.days)}
            className="px-2 py-1 text-[9px] font-black uppercase tracking-widest rounded-md border border-border dark:border-white/5 bg-surface-3 dark:bg-white/5 text-content-muted dark:text-content-dark-muted hover:border-brand-500 hover:text-brand-500 transition-all">
            {p.label}
          </button>
        ))}
      </div>
      <div className="flex items-center gap-2">
        <input type="date" value={from} onChange={e => onChange(e.target.value, to)}
          className="bg-surface-3 dark:bg-white/5 border border-border dark:border-white/5 text-[9px] font-black rounded-md py-1 px-2 text-content dark:text-white outline-none focus:ring-2 focus:ring-brand-500/20" />
...
export function KpiCard({ label, value, sub, icon, color = "text-brand-500", delta: d }) {
  return (
    <div className="rounded-xl border border-border dark:border-white/5 bg-white dark:bg-white/5 p-3 flex flex-col gap-1 shadow-sm transition-all group overflow-hidden">
      <div className="flex justify-between items-start">
        <div className="text-[9px] font-black text-content-muted dark:text-content-dark-muted uppercase tracking-[2px] leading-none">{label}</div>
      ...
      <div className="flex items-center justify-between gap-1 mt-0.5">
        {sub && <div className="text-[8px] font-bold text-content-muted dark:text-content-dark-muted opacity-60 truncate">{sub}</div>}
        {d !== null && d !== undefined && (
          <div className={`text-[8px] font-black px-1.5 py-0.5 rounded-full ${parseFloat(d) >= 0 ? "text-green-500 bg-green-500/10" : "text-danger bg-danger/10"}`}>
...
export function SectionHeader({ title, sub }) {
  return (
    <div className="flex flex-col gap-0.5 mb-2">
      <div className="text-[9px] font-black text-brand-500 uppercase tracking-[3px] leading-none">
        {title}
      </div>
      {sub && <div className="text-[8px] font-bold text-content-muted dark:text-content-dark-muted opacity-60 uppercase tracking-tight">{sub}</div>}
    </div>
  );
}
...
export function Loading() {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4">
      ...
      <div className="text-[10px] font-black text-content-muted dark:text-content-subtle uppercase tracking-[4px]">Cargando reporte...</div>
    </div>
  );
}
