...
export default function AuditReport() {
 ...
 <th key={h} className={`px-4 py-2 text-[11px] font-black uppercase tracking-wide text-content-muted dark:text-content-dark-muted ${i === 2 ? "text-right" : i === 3 ? "text-center" : ""}`}>{h}</th>
 ))}
 </tr>
 )}
 {view === "discounts" && (
 ...
 <td className="px-4 py-2 font-black text-[11px] uppercase tracking-wider text-content dark:text-white">{e.employee_name}</td>
 <td className="px-4 py-2 text-right tabular-nums font-black text-[11px] text-content-muted">{e.sale_count}</td>
 <td className="px-4 py-2 text-right tabular-nums text-green-500 font-black text-[11px]">{fmt$(e.revenue)}</td>
 <td className="px-4 py-2 text-right tabular-nums text-[11px] text-content-subtle">{fmt$(e.avg_ticket)}</td>
