        ...
        <div className="flex-1 min-h-0 space-y-3 overflow-auto">
          ...
          <Card className="!p-0 overflow-hidden min-h-0 flex flex-col">
            ...
            <div className="overflow-x-auto">
                  ...
                  {view === "top" && (
                    <tr className="border-b border-border/40 dark:border-white/5">
                      {["Cliente", "Compras", "Promedio", "Total Invertido", "Última"].map((h, i) => (
                        <th key={h} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest text-content-muted dark:text-content-dark-muted ${i >= 1 && i <= 3 ? "text-right" : i === 4 ? "text-center" : ""}`}>{h}</th>
                      ))}
                    </tr>
                  )}
                  {view === "inactive" && (
                    <tr className="border-b border-border/40 dark:border-white/5">
                      {["Cliente", "Valor Histórico", "Inactividad"].map((h, i) => (
                        <th key={h} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest text-content-muted dark:text-content-dark-muted ${i === 1 ? "text-right" : i === 2 ? "text-center" : ""}`}>{h}</th>
                      ))}
                    </tr>
                  )}
                </thead>
                <tbody className="divide-y divide-border/20 dark:divide-white/5">
                  {view === "top" && data.top_customers.map((c, i) => (
                    <tr key={i} className="hover:bg-surface-2 dark:hover:bg-white/[0.04] transition-colors">
                      <td className="px-4 py-2">
                        <div className="font-black text-[9px] uppercase tracking-wider text-content dark:text-white">{c.name}</div>
                        <div className="text-[8px] font-bold text-content-subtle opacity-50 uppercase">{c.phone || "Sin contacto"}</div>
                      </td>
                      <td className="px-4 py-2 text-right tabular-nums text-[9px] font-black text-content-muted">{c.purchase_count}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-[9px] text-content-subtle">{fmt$(c.avg_ticket)}</td>
                      <td className="px-4 py-2 text-right tabular-nums text-green-500 font-black text-[9px]">{fmt$(c.total_spent)}</td>
                      <td className="px-4 py-2 text-center text-[9px] font-black text-content-subtle uppercase">{new Date(c.last_purchase).toLocaleDateString("es-VE")}</td>
                    </tr>
                  ))}
                  {view === "inactive" && data.inactive_customers.map((c, i) => (
                    <tr key={i} className="hover:bg-surface-2 dark:hover:bg-white/[0.04] transition-colors">
                      ...
                      <td className="px-4 py-2 text-right tabular-nums text-brand-500 font-black text-[9px]">{fmt$(c.lifetime_value)}</td>
                      <td className="px-4 py-2 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${c.days_inactive > 60 ? "bg-danger/10 text-danger" : "bg-brand-500/10 text-brand-500"}`}>
                          {c.days_inactive}D
                        </span>
                      </td>
                    </tr>
              ...
              {(view === "new" || view === "ticket") && <div className="p-10 text-center text-[9px] font-black uppercase tracking-[3px] text-content-subtle opacity-20">Segmentación Completa en Reporte Maestro</div>}
            </div>
          </Card>
        </div>
      )}
