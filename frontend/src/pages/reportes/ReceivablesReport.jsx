          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={b.label} className={`${b.bg} rounded-xl p-3 border`}>
                <div className="text-[9px] font-black text-content-muted dark:text-content-dark-muted uppercase tracking-widest mb-0.5">{b.label}</div>
                <div className={`text-base font-black ${b.color} tabular-nums font-display`}>{fmt$(b.amount || 0)}</div>
              </div>
            ))}
          </div>
                ...
                <thead className="bg-surface-2 dark:bg-surface-dark-2/50">
                  <tr className="border-b border-border/40 dark:border-white/5">
                    {["Cliente", "Facturas", "Saldo Deudor", "Aging"].map((h, i) => (
                      <th key={h} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest text-content-muted dark:text-content-dark-muted ${i === 1 || i === 2 ? "text-right" : i === 3 ? "text-center" : ""}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={i} className="hover:bg-surface-2 dark:hover:bg-white/[0.04] transition-colors">
                        <td className="px-4 py-2">
                          <div className="font-black text-[9px] uppercase tracking-wider text-content dark:text-white">{c.customer_name}</div>
                          <div className="text-[8px] font-bold text-content-subtle opacity-50 uppercase">{c.phone || "—"}</div>
                        </td>
                        <td className="px-4 py-2 text-right tabular-nums text-[9px] font-black text-content-muted">{c.invoice_count}</td>
                        <td className="px-4 py-2 text-right tabular-nums text-danger font-black text-[9px]">{fmt$(c.balance)}</td>
                        <td className="px-4 py-2 text-center">
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${daysDiff > 60 ? "text-danger bg-danger/10" : daysDiff > 30 ? "text-brand-500 bg-brand-500/10" : "text-green-500 bg-green-500/10"}`}>
                            {daysDiff}D
                          </span>
                        </td>
                      </tr>
