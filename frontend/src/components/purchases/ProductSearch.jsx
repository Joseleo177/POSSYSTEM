    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="card-premium overflow-visible">
                <div className="text-[10px] font-black uppercase tracking-widest text-brand-500 mb-4 flex items-center gap-2">
                ...
                <div className="mb-6 relative z-[40]">
                    <label className="label !text-[10px] mb-1.5">Producto a Ingresar</label>

                    {itemForm.product ? (
                        <div className="flex items-center gap-4 bg-brand-500/[0.03] border border-brand-500/20 rounded-xl p-4 shadow-sm border-l-4 border-l-brand-500">
                            ...
                            <button
                                ...
                                className="h-9 px-4 flex items-center justify-center rounded-xl bg-danger/10 text-danger border border-danger/20 hover:bg-danger hover:text-white transition-all text-[10px] font-black uppercase tracking-widest active:scale-95"
                            >
                        ...
                        <div className="relative">
                            ...
                            {productSearch.trim().length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-surface-2 dark:bg-[#1a1c23] border border-border/30 dark:border-white/10 rounded-2xl shadow-2xl max-h-60 overflow-y-auto z-[100] p-1.5 backdrop-blur-xl">

                                    {productResults.map(p => (
                                        <div
                                            key={p.id}
                                            onClick={() => selectProduct(p)}
                                            className="px-4 py-3 cursor-pointer rounded-xl border border-transparent hover:border-brand-500/20 hover:bg-brand-500/5 transition-all group"
                                        >
                                            <div className="font-black text-brand-500 group-hover:text-brand-400 uppercase text-xs">
                                                {p.name}
                                            </div>

                                            <div className="text-[10px] font-bold text-content-subtle opacity-50 mt-1 uppercase whitespace-nowrap overflow-hidden text-ellipsis tabular-nums">
                                    ...
                                    <div
                                        ...
                                        className={`mt-1 px-4 py-3 cursor-pointer rounded-xl bg-warning/5 border border-warning/10 text-[11px] font-black text-warning flex items-center gap-3 hover:bg-warning hover:text-black transition-all group`}
                                    >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className={`input h-10 text-center font-black tabular-nums transition-all ${itemForm.package_unit?.toLowerCase() === "unidad"
                                    ? "bg-surface-3 opacity-30 cursor-not-allowed border-dashed"
                                    : ""
                                    }`}
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className="input h-10 text-center font-black tabular-nums border-brand-500/20 active:border-brand-500 focus:border-brand-500"
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                placeholder="0.00"
                                className="input h-10 text-center font-black tabular-nums text-brand-500"
                            />
                        </div>
                    </div>

                    {/* Sección de Cálculos y Proyecciones */}
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                        <div className="bg-surface-1 dark:bg-white/[0.02] p-4 rounded-xl border border-border/10">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-content-subtle mb-3 block">Margen Sugerido</label>
                            <div className="relative">
                                <input
                                    type="number"
                                    min="0"
                                    ...
                                    className="input h-10 pr-8 font-bold tabular-nums"
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-content-subtle opacity-40">%</span>
                            </div>
                        </div>

                        <div className="bg-surface-1 dark:bg-white/[0.02] p-4 rounded-xl border border-border/10 flex flex-col justify-between">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-content-subtle mb-1 block">Costo Unitario</label>
                            <div className="text-xl font-black text-info tabular-nums tracking-tighter">
                                <span className="text-xs mr-1 opacity-40">$</span>
                                {calc?.unit_cost ? fmt2(calc.unit_cost) : "0"}
                            </div>
                            <div className="text-[8px] font-bold text-info/50 uppercase tracking-widest leading-none mt-1">Cálculo Proyectado</div>
                        </div>

                        <div className="bg-brand-500/5 p-4 rounded-xl border border-brand-500/10 flex flex-col justify-between">
                            ...
                            <div className="text-[8px] font-bold text-brand-500/50 uppercase tracking-widest leading-none mt-1 italic">Sincronización Activa</div>
                        </div>

                        <div className="bg-surface-1 dark:bg-white/[0.02] p-4 rounded-xl border border-border/10 flex flex-col justify-between">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-warning mb-1 block">Total Unidades</label>
                            <div className="text-xl font-black text-warning tabular-nums tracking-tighter">
                                {calc?.total_units || "0"}
                                <span className="text-[10px] ml-1.5 opacity-40 uppercase">{itemForm.product?.unit || "uds"}</span>
                            </div>
                            <div className="text-[8px] font-bold text-warning/50 uppercase tracking-widest leading-none mt-1">Ingreso al Stock</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className="input h-10 font-bold tracking-widest uppercase tabular-nums"
                            />
                        </div>
                        <div className="space-y-1.5">
                ...
                <div className="mt-8 pt-6 border-t border-border/20 dark:border-white/5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <button 
                        ...
                        className="h-10 px-8 rounded-xl bg-success text-black text-[11px] font-bold uppercase tracking-widest hover:brightness-105 disabled:opacity-30 disabled:grayscale transition-all shadow-lg shadow-success/10 active:scale-95 flex items-center justify-center gap-2"
                    >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
                        Confirmar Entrada
                    </button>
            ...
            {items.length > 0 && (
                <div className="card-premium !p-0 overflow-hidden shadow-2xl">
                    <div className="px-5 py-3 bg-surface-2 dark:bg-white/[0.01] border-b border-border/10 dark:border-white/5 flex items-center justify-between">
                                    ...
                                    <tr key={item.key} className="group hover:bg-brand-500/[0.02] transition-colors">
                                        ...
                                        <td className="text-right pr-6">
                                            <button
                                                ...
                                                className="w-8 h-8 flex items-center justify-center rounded-xl bg-danger/5 text-danger opacity-0 group-hover:opacity-100 hover:bg-danger hover:text-white transition-all active:scale-95"
                                            >
                    ...
                    {/* Totales y AcciÃ³n final */}
                    <div className="bg-surface-2 dark:bg-white/[0.04] p-6 border-t border-border/30 dark:border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-content-subtle opacity-40 leading-none mb-1">Total del Recibo</div>
                                <div className="text-3xl font-black text-brand-500 tabular-nums tracking-tighter leading-none">${fmt2(grandTotal)}</div>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            className={`h-12 px-10 rounded-2xl flex items-center gap-3 transition-all active:scale-95 shadow-2xl ${loading || !selectedWarehouseId
                        ...
                        >
                            <span className="text-[11px] font-black uppercase tracking-widest">
                                {loading ? "Procesando Almacen..." : !selectedWarehouseId ? "Esperando Alma©n" : "Finalizar y Cargar Stock"}
                            </span>
            ...
            {items.length === 0 && (
                <div className="card-premium flex flex-col items-center justify-center py-12 gap-3 opacity-20 grayscale border-dashed">
                    <svg className="w-12 h-12 text-content-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                    <div className="text-[10px] font-black uppercase tracking-widest text-center">El recibo esta vaci­o.<br />Agrega productos para registrar la compra.</div>
                </div>
            )}
        </div>
    );
