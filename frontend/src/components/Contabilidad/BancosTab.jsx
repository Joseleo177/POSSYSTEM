...
export default function BancosTab({
 ...
 <div className="w-10 rounded-xl bg-brand-500/10 text-brand-500 flex items-center justify-center text-xl"></div>
 <div>
 <div className="text-[11px] font-black text-content-subtle uppercase tracking-wide">CONFIGURACIÓN DE ENTIDADES</div>
 <div className="text-xs font-black text-content dark:text-content-dark">Bancos e Instituciones Financieras</div>
 </div>
 </div>
 {can("config") && (
 <button
 ...
 <div className="text-xs font-black uppercase tracking-wide text-content-muted">Sin bancos registrados</div>
 </div>
 ) : (
 <div className="bg-surface-2 dark:bg-surface-dark-3 rounded-2xl overflow-hidden border border-border/40 mb-5 shadow-sm">
 <table className="w-full text-sm border-collapse">
 <thead>
 <tr className="bg-surface-3 dark:bg-surface-dark border-b border-border/40">
 <th className="text-left px-4 py-2.5 font-black text-content-subtle uppercase tracking-wider text-xs">Nombre del Banco</th>
 <th className="text-left px-4 py-2.5 font-black text-content-subtle uppercase tracking-wider text-xs">Código</th>
 <th className="text-center px-4 py-2.5 font-black text-content-subtle uppercase tracking-wider text-xs">Cuentas/Diarios</th>
 <th className="text-center px-4 py-2.5 font-black text-content-subtle uppercase tracking-wider text-xs">Estado</th>
 {can("config") && <th className="text-right px-4 py-2.5 font-black text-content-subtle uppercase tracking-wider text-xs w-48">Acciones</th>}
 </tr>
 </thead>
 ...
 <td className="px-4 py-2 text-center">
 <span className={`px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wide border ${b.active ? 'bg-success/5 text-success border-success/20' : 'bg-danger/5 text-danger border-danger/20'}`}>
 ...
 {isEdit ? (
 <>
 <button onClick={saveBank} disabled={bankSaving} className="p-2.5 bg-success/10 text-success rounded-xl hover:bg-success hover:text-white transition-all shadow-sm">
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
 </button>
 <button onClick={() => { setBankEditId(null); setBankForm(EMPTY_BANK); }} className="p-2.5 bg-surface-3 text-content-muted rounded-xl hover:bg-content hover:text-white transition-all shadow-sm">
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg>
 </button>
 </>
 ) : (
 <>
 <button onClick={() => { setBankEditId(b.id); setBankForm({ name: b.name, code: b.code || "", sort_order: b.sort_order ?? 0 }); }} className="p-2.5 bg-brand-500/10 text-brand-500 rounded-xl hover:bg-brand-500 hover:text-black transition-all shadow-sm" title="Editar">
 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
 </button>
 <button onClick={() => toggleBank(b)} className="p-2.5 bg-info/10 text-info rounded-xl hover:bg-info hover:text-white transition-all shadow-sm" title={b.active ? "Desactivar" : "Activar"}>
 ...
 <button onClick={() => setDeleteConfirm(b)} className="p-2.5 bg-danger/10 text-danger rounded-xl hover:bg-danger hover:text-white transition-all shadow-sm" title="Eliminar">
 ...
 onChange={e => setBankForm(p => ({ ...p, code: e.target.value }))}
 placeholder="ej. 0102"
 className="input-pos bg-white dark:bg-surface-dark border-none py-2 px-3 rounded-xl text-sm font-bold w-full font-mono"
 onKeyDown={e => e.key === "Enter" && saveBank()}
 />
 </div>
 </div>
 ...
 onClick={saveBank}
 disabled={bankSaving}
 className="w-full py-3 rounded-xl bg-brand-500 text-black font-black text-[11px] uppercase tracking-wide shadow-lg hover:shadow-brand-500/20 transition-all border-none cursor-pointer"
 >
