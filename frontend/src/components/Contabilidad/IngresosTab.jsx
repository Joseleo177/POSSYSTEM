      ...
      <div className="shrink-0 card-premium p-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex-1 max-w-lg">
            <div className="text-[10px] font-black text-brand-500 uppercase tracking-[2px] mb-2">Periodo de Análisis</div>
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <span className="text-[9px] font-black text-content-muted uppercase tracking-widest mb-1 block">Desde</span>
                <input
                  type="date"
                  value={histDateFrom}
              ...
              <div className="flex-1">
                <span className="text-[9px] font-black text-content-muted uppercase tracking-widest mb-1 block">Hasta</span>
                <input
                  type="date"
                  value={histDateTo}
        ...
        {summaryView === "diarios" && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-[2px] text-content dark:text-white">Distribución por Diarios</span>
            </div>
            <JournalSummary
          ...
          return (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-[2px] text-content dark:text-white">Distribución por Bancos</span>
              </div>
                    ...
                    <div key={bank} className="bg-white dark:bg-surface-dark-3 rounded-xl p-4 border border-border dark:border-border-dark shadow-sm">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          ...
                          <div className="text-[9px] font-bold text-content-muted uppercase tracking-widest">Consolidado Bancario</div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {d.journals.map(j => (
                          <span key={j.id} className="px-2.5 py-1 bg-surface-2 dark:bg-white/5 text-[9px] font-black text-content dark:text-white rounded-lg border border-border/40 dark:border-white/10">
                            {j.name}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-2.5 border-t border-border/20 dark:border-white/5">
                        <div>
                          <div className="text-[9px] font-black text-content-muted uppercase tracking-widest">Total TX</div>
                          <div className="text-xs font-black text-content dark:text-white">{countPagos} pagos</div>
                        </div>
                        <div className="text-right">
                          <div className="text-[9px] font-black text-success uppercase tracking-widest">Hoy</div>
        ...
        {summaryView === "series" && (() => {
          ...
          return (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="h-3 w-1 bg-brand-500 rounded-full" />
                <span className="text-[10px] font-black uppercase tracking-[2px] text-content dark:text-white">Distribución por Series</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {entries.map(([serie, d]) => (
                  <div key={serie} className="bg-white dark:bg-surface-dark-3 border border-brand-500/20 rounded-xl p-4 shadow-sm">
                    <div className="text-[10px] font-black text-brand-500 uppercase tracking-[2px] mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-brand-500 shrink-0" />
                      {serie}
                    </div>
                    <div className="mb-3">
                      <div className="text-[9px] font-black text-content-muted uppercase tracking-widest mb-0.5">Total Facturado</div>
                    ...
                    <div className="flex items-center gap-2 py-1.5 px-3 bg-surface-2 dark:bg-white/5 rounded-lg w-fit border border-border/30 dark:border-white/10">
                      ...
                      <span className="text-[9px] font-black text-content-muted uppercase tracking-widest">Documentos</span>
                    </div>
                  </div>
                ))}
              </div>
