                      ...
                      {can("config") && (
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            {isEdit ? (
                              <>
                                <button onClick={saveJournal} className="p-2.5 bg-success/10 text-success rounded-xl hover:bg-success hover:text-white transition-all shadow-sm" title="Guardar">✓</button>
                                <button onClick={() => setEditJournal(null)} className="p-2.5 bg-surface-3 text-content-muted rounded-xl hover:bg-content hover:text-white transition-all shadow-sm">✕</button>
                              </>
                            ) : (
                              <>
                                <button onClick={() => setEditJournal({...j})} className="p-2.5 bg-brand-500/10 text-brand-500 rounded-xl hover:bg-brand-500 hover:text-black transition-all shadow-sm" title="Editar">✎</button>
                                <button onClick={() => toggleJournal(j)} className="p-2.5 bg-info/10 text-info rounded-xl hover:bg-info hover:text-white transition-all shadow-sm" title={j.active ? "Desactivar" : "Activar"}>⏻</button>
                                <button onClick={() => deleteJournal(j.id)} className="p-2.5 bg-danger/10 text-danger rounded-xl hover:bg-danger hover:text-white transition-all shadow-sm" title="Eliminar">🗑</button>
                              </>
                            )}
                          </div>
                        </td>
