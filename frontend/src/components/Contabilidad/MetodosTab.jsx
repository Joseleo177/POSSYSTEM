      ...
      <div className="bg-white dark:bg-surface-dark-2 border border-border dark:border-border-dark rounded-2xl p-6 mb-8 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-success/10 text-success flex items-center justify-center text-xl">💳</div>
            <div>
                        ...
                        <div className="flex items-center gap-3">
                          {isEdit ? (
                            <input 
                              ...
                              className="w-8 h-8 p-1 bg-white border-none rounded-lg cursor-pointer" 
                            />
                          ) : (
                          ...
                          {isEdit ? (
                            <input 
                              ...
                              onChange={e => setMethodForm(p => ({...p, icon: e.target.value}))}
                              className="input-pos bg-white border-none py-1 w-10 text-center text-lg rounded-lg outline-none focus:ring-2 focus:ring-brand-500/20" 
                            />
                          ) : (
                      ...
                      {can("config") && (
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            {isEdit ? (
                              <>
                                <button onClick={saveMethod} disabled={methodSaving} className="p-2.5 bg-success/10 text-success rounded-xl hover:bg-success hover:text-white transition-all shadow-sm">✓</button>
                                <button onClick={() => { setMethodEditId(null); setMethodForm(EMPTY_METHOD); }} className="p-2.5 bg-surface-3 text-content-muted rounded-xl hover:bg-content hover:text-white transition-all shadow-sm">✕</button>
                              </>
                            ) : (
                              <>
                                <button onClick={() => { setMethodEditId(m.id); setMethodForm({ name:m.name, code:m.code, icon:m.icon||"💳", color:m.color||"#555555", sort_order:m.sort_order??0 }); }} className="p-2.5 bg-brand-500/10 text-brand-500 rounded-xl hover:bg-brand-500 hover:text-black transition-all shadow-sm" title="Editar">✎</button>
                                <button onClick={() => toggleMethod(m)} className="p-2.5 bg-info/10 text-info rounded-xl hover:bg-info hover:text-white transition-all shadow-sm" title={m.active ? "Desactivar" : "Activar"}>⏻</button>
                                <button onClick={() => removeMethod(m)} className="p-2.5 bg-danger/10 text-danger rounded-xl hover:bg-danger hover:text-white transition-all shadow-sm" title="Eliminar">🗑</button>
                              </>
                            )}
                          </div>
                        </td>
          ...
          >
            ...
            <div className="p-8">
              <div className="grid grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  <div className="text-[10px] font-black text-content-subtle uppercase tracking-widest mb-2 ml-1">Ícono / Emoji</div>
                  <input 
                    value={methodForm.icon} 
                    onChange={e => setMethodForm(p => ({...p, icon: e.target.value}))}
                    className="input-pos bg-white dark:bg-surface-dark border-none py-3 px-4 rounded-xl text-lg text-center w-full" 
                  />
                </div>
                <div>
