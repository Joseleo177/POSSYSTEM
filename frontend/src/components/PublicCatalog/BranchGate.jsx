...
// Selección de sucursal: primera pantalla cuando la tienda tiene más de un local que vende.
//
// Va antes que la identificación a propósito. La sucursal decide qué catálogo se ve —los
// precios son de la empresa, pero el stock es de cada local— y dejar entrar sin elegirla
// mostraría el inventario sumado de todas: el cliente pediría algo que en la tienda a la que
// va no está. Con una sola sucursal esta pantalla no aparece.
                        ...
                        {warehouses.map(w => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full p-3.5 rounded-2xl flex items-center gap-3 text-left border active:scale-[0.99] transition-all ${actual
                                        ? "bg-brand-500/10 border-brand-500/60"
                                        : "bg-surface-2 dark:bg-white/5 border-border dark:border-white/10 hover:border-brand-500/60"
                                        }`}
                                >
                                    ...
                                    <span className="flex-1 min-w-0">
                                        <span className="block text-[13px] font-black uppercase tracking-tight text-content dark:text-white truncate">
                                            {w.name}
                                        </span>
                                        {actual && (
                                            <span className="block text-[10px] font-black uppercase tracking-widest text-brand-500">
                                                Estás aquí
                                            </span>
                                        )}
                                    </span>
                                    <svg className="w-4 h-4 text-content-subtle shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                                </button>
                            );
                        })}
                    </div>
