        ...
        <div className="h-full flex flex-col">
            ...
            <div className="shrink-0 px-4 py-1.5 flex items-center gap-1 border-b border-border/20 dark:border-white/5 bg-surface-2/50 dark:bg-white/[0.02]">
                {[["almacenes", "Almacenes"], ["transferencias", "Transferencias"]].map(([key, label]) => (
                    <button
                        ...
                        className={`px-4 py-1 rounded-full text-[11px] font-black uppercase tracking-wide whitespace-nowrap transition-all ${subTab === key || (key === "almacenes" && subTab === "stock")
            ...
            <Modal open={transferModal} onClose={() => { setTransferModal(false); setTransferForm({ from_warehouse_id: "", to_warehouse_id: "", product_id: "", qty: "", note: "" }); }} title="Nueva Transferencia" width={520}>
                ...
                <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-border/10">
                    <button onClick={() => { setTransferModal(false); setTransferForm({ from_warehouse_id: "", to_warehouse_id: "", product_id: "", qty: "", note: "" }); }} className="btn-md btn-secondary px-6">Cancelar</button>
                    <button onClick={doTransfer} disabled={loadingTransfer} className={`btn-md px-6 ${loadingTransfer ? "btn-secondary opacity-60 cursor-not-allowed" : "btn-primary"}`}>
            ...
            <Modal open={warehouseModal} onClose={cancelEdit} title={editId ? "Editar Almacén" : "Nuevo Almacén"} width={480}>
                ...
                <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-border/10">
                    <button onClick={cancelEdit} className="btn-md btn-secondary px-6">Cancelar</button>
                    <button onClick={saveWarehouse} disabled={loading} className="btn-md btn-primary px-6">
