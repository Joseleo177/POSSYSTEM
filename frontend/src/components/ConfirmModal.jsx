...
export default function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[1050] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="w-full max-w-[400px] bg-white dark:bg-surface-dark-2 border border-border dark:border-white/5 rounded-[32px] p-8 shadow-2xl relative overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 ease-out">
        {/* Glow de fondo */}
        <div className="absolute -top-20 -right-20 w-48 h-48 bg-brand-500/5 rounded-full blur-3xl pointer-events-none" />
        
        <div className="flex flex-col items-center text-center relative z-10">
          <div className={`mb-6 w-20 h-20 rounded-[24px] flex items-center justify-center shadow-inner ${config.iconContainer}`}>
            {config.icon}
          </div>

          <h3 className={`text-xl font-black tracking-widest mb-2 uppercase font-display ${config.titleClass}`}>
            {title}
          </h3>

          <p className="text-sm text-content-muted dark:text-content-dark-muted leading-relaxed mb-8 font-medium">
            {message}
          </p>
          ...
          <div className="grid grid-cols-2 w-full gap-3">
            <button
              onClick={onCancel}
              className="h-12 bg-surface-2 dark:bg-white/5 text-content-subtle dark:text-content-dark-muted rounded-2xl w-full font-black text-[10px] uppercase tracking-[3px] hover:bg-surface-3 dark:hover:bg-white/10 transition-all active:scale-95 border-none"
            >
              {cancelText}
            </button>
            <button
              onClick={onConfirm}
              className={`h-12 rounded-2xl w-full text-[10px] font-black uppercase tracking-[3px] transition-all active:scale-95 ${config.btnClass}`}
            >
              {confirmText}
            </button>
          </div>
