                            ...
                            <thead>
                                <tr className="bg-surface-2 dark:bg-surface-dark-2">
                                    {["Producto", "Categoría", "Stock Actual", "P. Venta", "Acciones"].map(h => (
                                        <th key={h} className="px-4 py-2.5 text-[10px] font-black uppercase tracking-wide text-content-subtle border-b border-border/40 dark:border-white/5">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                                    ...
                                    <tr>
                                        <td colSpan={5} className="px-4 py-16 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <div className="text-content-subtle text-[11px] font-bold uppercase tracking-wide">No se encontraron productos</div>
                                                <button onClick={openAddStock} className="text-brand-500 font-black text-[10px] uppercase tracking-wide underline underline-offset-2">Agregar Stock</button>
                                            </div>
                                        </td>
                                    </tr>
                                ) : filteredStock.map(s => (
                                    <tr key={s.product_id} className="group hover:bg-brand-500/[0.02] transition-colors">
                                        <td className="px-4 py-2">
                                            <div className="flex flex-col">
                                                <span className="text-xs font-black text-content dark:text-heading-dark tracking-tight uppercase group-hover:text-brand-500 transition-colors">{s.product_name}</span>
                                                {s.is_combo && <span className="w-fit mt-0.5 px-1.5 py-px bg-indigo-500/10 text-indigo-400 text-[9px] font-black uppercase tracking-wide rounded">Combo Virtual</span>}
                                            </div>
                                        </td>
                                        <td className="px-4 py-2">
                                        ...
                                        <td className="px-4 py-2">
                                            <span className={`text-sm font-black tabular-nums ${parseFloat(s.qty) <= 0 ? "text-danger" : parseFloat(s.qty) <= 5 ? "text-warning" : "text-success"}`}>
                                                {fmtQty(s.qty)}
                                                <span className="text-[9px] ml-1 opacity-50 uppercase">{s.unit || "uds"}</span>
                                            </span>
                                        </td>
                                        <td className="px-4 py-2 font-black text-brand-500 text-xs tabular-nums">${parseFloat(s.price || 0).toFixed(2)}</td>
                                        <td className="px-4 py-2">
                                            <div className="flex items-center gap-1.5">
                                                <button
                                                    onClick={() => handleEditStock(s)}
                                                    className="p-1.5 rounded-lg bg-info/10 text-info border border-info/20 hover:bg-info hover:text-black transition-all"
                                                    title="Ajustar"
                                                >
