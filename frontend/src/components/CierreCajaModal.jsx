      ...
      <div className="w-full max-w-lg bg-surface-2 dark:bg-[#141414] border border-border/40 dark:border-white/10 rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-warning/10 border-b border-warning/20 px-7 py-5 flex items-center gap-4 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[10px] font-black uppercase tracking-[3px] text-warning/70">CIERRE DE TURNO</div>
            <div className="text-lg font-black text-content dark:text-content-dark">Cierre de Caja</div>
            <div className="text-[10px] text-content-subtle font-bold mt-0.5">{session.warehouse?.name} · {session.employee?.full_name}</div>
          </div>
          <div className="text-right shrink-0">
            <div className="text-[9px] text-content-subtle font-bold uppercase">Duración</div>
            <div className="text-base font-black text-warning">{durationLabel}</div>
          </div>
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-surface-3 dark:bg-white/5 rounded-2xl p-3 text-center">
                  <div className="text-[9px] font-black uppercase tracking-widest text-content-subtle mb-1">Ventas</div>
                  <div className="text-xl font-black text-content dark:text-content-dark">{summary.sales.sale_count}</div>
                </div>
                <div className="bg-surface-3 dark:bg-white/5 rounded-2xl p-3 text-center">
                  <div className="text-[9px] font-black uppercase tracking-widest text-content-subtle mb-1">Facturado</div>
                  <div className="text-sm font-black text-success">{fmt$(summary.sales.total_sales)}</div>
                </div>
                <div className="bg-surface-3 dark:bg-white/5 rounded-2xl p-3 text-center">
                  <div className="text-[9px] font-black uppercase tracking-widest text-content-subtle mb-1">Pendientes</div>
              ...
              {summary.payments_by_journal.length > 0 && (
                <div className="bg-surface-3 dark:bg-white/5 rounded-2xl p-4">
                  <div className="text-[10px] font-black uppercase tracking-widest text-content-subtle mb-3">Todos los Cobros</div>
                  {summary.payments_by_journal.map((p, i) => (
                    <div key={i} className="flex items-center justify-between py-2 border-b border-border/10 dark:border-white/5 last:border-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-content-subtle">{p.payment_count} cobros</span>
                      </div>
              ...
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-content-subtle mb-3">Cuadre de Efectivo por Caja</div>
                      ...
                      <div key={j.journal_id} className="bg-surface-3 dark:bg-white/5 rounded-2xl p-4">
                        ...
                        <div className="flex flex-col gap-3 mb-4 bg-black/5 dark:bg-white/5 rounded-xl p-3 border border-border/20 dark:border-white/5">
                          <div className="flex justify-between items-center text-xs">
                            <span className="font-black text-content-subtle uppercase tracking-widest text-[9px]">Fondo Inicial</span>
                            <span className="font-black tabular-nums">{f(j.opening_amount)}</span>
                          </div>
                          <div className="flex justify-between items-center text-xs">
                            <span className="font-black text-content-subtle uppercase tracking-widest text-[9px]">Ingresos (Cobros)</span>
                          ...
                          <div className="flex justify-between items-center py-2 border-t border-border/10 dark:border-white/10">
                            <span className="font-black text-content-subtle uppercase tracking-widest text-[9px]">Total Esperado</span>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full bg-white/5 border-2 border-warning/30 rounded-xl px-4 py-2.5 pr-8 text-base font-black text-warning outline-none focus:ring-2 focus:ring-warning/30 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none transition-all"
                          />
              ...
              {/* Notas */}
              <div>
                <label className="block text-[10px] font-black uppercase tracking-[2px] text-content-muted dark:text-content-dark-muted mb-2">Notas (opcional)</label>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-border/20 dark:border-white/5 flex gap-3 shrink-0">
          <button onClick={onCancel}
            className="flex-1 py-4 rounded-2xl border border-border dark:border-white/10 font-black text-[11px] uppercase tracking-widest text-content-muted dark:text-content-dark-muted hover:bg-surface-3 dark:hover:bg-white/5 transition-all cursor-pointer">
            Cancelar
          </button>
          <button onClick={handleClose} disabled={saving || loadingSummary || !allFilled}
            className={`flex-[2] py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all cursor-pointer
