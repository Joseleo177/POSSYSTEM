  ...
  if (restrictToWarehouse) {
    ...
    if (stock_filter === 'with') {
      andClauses.push(Sequelize.literal(`(
        EXISTS (SELECT 1 FROM product_stock ps WHERE ps.product_id = "Product"."id" AND ps.warehouse_id = ${wid} AND ps.qty > 0)
        OR "Product"."is_combo" = true
        OR "Product"."is_service" = true
      )`));
