  ...
  const [byProduct, byCategory, summary, byDay] = await Promise.all([
    sequelize.query(
      `SELECT
         ...
         COALESCE(SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0)), 0)::float AS total_cost,
         COALESCE(SUM(si.subtotal) - SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0)), 0)::float AS gross_margin,
         CASE WHEN SUM(si.subtotal) > 0
              THEN ROUND(((SUM(si.subtotal) - SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0))) / SUM(si.subtotal) * 100)::numeric, 1)
              ELSE 0
         END AS margin_pct
       FROM sale_items si
    ...
    sequelize.query(
      `SELECT
         ...
         COALESCE(SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0)), 0)::float AS total_cost,
         COALESCE(SUM(si.subtotal) - SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0)), 0)::float AS total_margin,
         CASE WHEN SUM(si.subtotal) > 0
              THEN ROUND(((SUM(si.subtotal) - SUM(si.quantity * COALESCE(si.cost_price, p.cost_price, 0))) / SUM(si.subtotal) * 100)::numeric, 1)
              ELSE 0
