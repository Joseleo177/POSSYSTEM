...
exports.getCategories = async (req, res, next) => {
  try {
    const cats = await IncomeCategory.findAll({ where: { active: true }, order: [['name', 'ASC']] });
...
exports.upsertCategory = async (req, res, next) => {
  try {
    ...
    let cat;
    if (id) {
      cat = await IncomeCategory.findByPk(id);
      ...
      cat = await IncomeCategory.create({ name, active: true });
    }
...
exports.voidIncome = async (req, res, next) => {
  try {
